...
;// CONCATENATED MODULE: ./apis/home/http/testLogo.js
...
const getTestLogo = params => {
  params.tags.name = 'getTestLogo';
  params.headers['Content-Type'] = 'application/json';
  const resp = http_default().get(`${__ENV.LB_HOST}/ms-gateway/tix-home-admin/logo/all`, params);
  return resp;
};
...
;// CONCATENATED MODULE: ./generator/home/testLogo.js
...
const Logo = () => {
  ...
  let accountId = Math.random().toString(8).slice(12);
  let requestId = Math.random().toString(8).slice(12);
  let username = Math.random().toString(36).slice(2) + Math.random().toString(36).slice(2);
  const params = {
    headers: {
      lang: 'id',
      accept: '*/*',
  ...
  (0,external_k6_namespaceObject.group)('logo', () => {
    const l = getTestLogo(params);
    assertStatus(l, 200, true, 'getTestLogo');
    assertSuccess(l, 'SUCCESS', true, 'geTesttLogo');
  });
};
;// CONCATENATED MODULE: ./scenarios/home/testLog.js


function testLog_distribute(value) {
...
const config = {
  ...
  groupServices: {
    Logo: {
      targetRate: testLog_distribute(2 * __ENV.X_TARGET)
    }
  }
};
const options = {
  ...
  if (config.scenario === 'verify') {
    ...
    options.scenarios[name] = {
      ...
      stages: [{
        ...
        duration: '1m',
        target: config.groupServices[name].targetRate * 1
      }, // { duration: '1m', target: config.groupServices[name].targetRate*1 },
      // { duration: '1m', target: config.groupServices[name].targetRate*1 },
      // { duration: '1m', target: config.groupServices[name].targetRate*1 },
      // { duration: '1m', target: config.groupServices[name].targetRate*1 },
      // { duration: '1m', target: config.groupServices[name].targetRate*1 },
