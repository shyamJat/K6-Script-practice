...
export { checkVersion, paymentLanding, paymentGroup, paymentMethod, promoSuggestion,paymentConfirm } from '@generator/payment/core/paymentCoreGQL.js'
export { applyUnapply,PaymentSubmit, applyUnapplyPaymentSubmit } from '@generator/payment/core/paymentCoreGQL.js'
...
const config = {
  ...
  groupServices: {
    ...
    checkVersion: { targetRate: distribute(serverCount, 2560), dist: 1 },
    paymentGroup: { targetRate: distribute(serverCount, 380), dist: 1 },
    paymentMethod: { targetRate: distribute(serverCount, 400), dist: 1 },
    paymentLanding: { targetRate: distribute(serverCount, 2050), dist: 1 },// 3190-1140
    promoSuggestion: { targetRate: distribute(serverCount, 680), dist: 1 },
    applyUnapplyPaymentSubmit: { targetRate: distribute(serverCount, 1140), dist: 1 },//creating order, payment with method,apply, unapply  and payment-submit calls
    paymentConfirm: { targetRate: distribute(serverCount, 680), dist: 1 },
  ...
  if (config.scenario === 'verify') {
    ...
    if (config.scenario === 'createorder') {
      ...
      stages = [
        ...
        { duration: '10s', target: Math.round(config.groupServices[name].targetRate * 1) },
        { duration: '3m', target: Math.round(config.groupServices[name].targetRate * 1) },
        ...
        { duration: '20m', target: Math.round(config.groupServices[name].targetRate * 1.5) },
