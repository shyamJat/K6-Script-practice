...
charactersLength));
  ...
  //promoSuggestion(referenceId,orderHash,userName,channelId,requestId);
  ...
  applyPaymentInquiryFunc(channelId,requestId,referenceId,userName);

  //const orderDetailId=applyDiscount.json().data.orderDetailId;
  const orderDetailId=21473186;
  //promoUnApply(referenceId, orderHash, totalCustomerPrice, orderDetailId, userName,channelId);
  ...
  paymentConfirm(referenceId,orderHash,userName,channelId,requestId);
...
const creditCardcheckPaidStatusFunc =(channelId,requestId,username,referenceId)=>{
  ...
  const applyPaymentInquiryFunc = (channelId,requestId,referenceId,username) => {
    ...
    group('apply-put-payment-inquiry', () => {
    const hp = ApplyPaymentInquiry(host, params, body,referenceId)
    assertStatus(hp, 200, true, 'apply-put-payment-inquiry')
    assertSuccess(hp, 'SUCCESS', true, 'apply-put-payment-inquiry')
    })
  }
  ...
  const unapplyPaymentInquiryFunc = (channelId,requestId,referenceId,username) => {
    ...
    group('unapply-put-payment-inquiry', () => {
    const hp = UnapplyPaymentInquiry(host, params, body,referenceId)
    assertStatus(hp, 200, true, 'unapply-put-payment-inquiry')
    assertSuccess(hp, 'SUCCESS', true, 'unapply-put-payment-inquiry')
    })
  }