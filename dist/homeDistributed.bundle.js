...
;// CONCATENATED MODULE: ./generator/home/homePages.js
...
;// CONCATENATED MODULE: ./scenarios/home/superApi.js
...
function superApi_distribute(serverCount, value) {
  const percentage = 100 / serverCount;
  return Math.round(percentage / 100 * value);
}
...
const config = {
  ...
  groupServices: {
    generalConfig: {
      targetRate: superApi_distribute(serverCount, 100000),
      dist: 1
    },
    homePageNonLogin: {
      targetRate: superApi_distribute(serverCount, 55000),
      dist: 1
    },
    homePageLogin: {
      targetRate: superApi_distribute(serverCount, 45000),
      dist: 1
    },
    homeModules: {
      targetRate: superApi_distribute(serverCount, 7272),
      dist: 1
    },
    heroBanner: {
      targetRate: superApi_distribute(serverCount, 5180),
      dist: 1
    },
    homeVerticals: {
      targetRate: superApi_distribute(serverCount, 5300),
      dist: 1
    },
    logo: {
      targetRate: superApi_distribute(serverCount, 24312),
      dist: 1
    },
    userFooter: {
      targetRate: superApi_distribute(serverCount, 18400),
      dist: 1
    },
    userVertical: {
      targetRate: superApi_distribute(serverCount, 17900),
      dist: 1
    }
  }
};
...
for (let i = 0; i < services.length; i++) {
  ...
  if (config.scenario === 'verify') {
    ...
    options.scenarios[name] = {
      ...
      //   { duration: '2m', target: 0 }
      // ]
      // #1 #2 #3 #4: all test that target 100k RPM
      // stages: [
      ...
      // ]
      // Perf break point stages
      stages: [// { duration: '2m', target: Math.round(config.groupServices[name].targetRate * 0.1) },
      // { duration: '1m', target: Math.round(config.groupServices[name].targetRate * 0.1) },
      // { duration: '2m', target: Math.round(config.groupServices[name].targetRate * 0.2) },
      // { duration: '1m', target: Math.round(config.groupServices[name].targetRate * 0.2) },
      // { duration: '2m', target: Math.round(config.groupServices[name].targetRate * 0.3) },
