...
const config = {
  ...
  groupServices: {
    ...
    // paymentFlow2: { targetRate: distribute(serverCount, 1900), dist: 1 },
      paymentFlow3: { targetRate: distribute(serverCount, 1000), dist: 1 },
  ...
  if (config.scenario === 'verify') {
    ...
    if (config.scenario === 'createorder') {
      ...
      stages = [
        ...
        { duration: '1m', target: Math.round(config.groupServices[name].targetRate * 2.5) },      //17 minutes
        ...
        { duration: '2m', target: Math.round(config.groupServices[name].targetRate * 3.7) },
        { duration: '50m', target: Math.round(config.groupServices[name].targetRate * 3.7) },
